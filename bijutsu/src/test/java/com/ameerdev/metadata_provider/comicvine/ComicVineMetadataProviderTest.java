...
@QuarkusTest
@Disabled
class ComicVineMetadataProviderTest {
    @Inject
    ComicVineMetadataProvider metadataProvider;

    @Test
    void metadataType() {
        assertEquals(BookType.COMIC, metadataProvider.metadataType());
    }
...
//                .thenReturn(mockResponse);
        ...
        String result = metadataProvider.search(SeriesIdentifier.builder().title("Batman").build()).orElseThrow();
    ...
    void fetchSeriesMetadata() {
        ...
        Series result = metadataProvider.fetchSeriesMetadata(metadataId).orElseThrow();
    ...
    void fetchReleaseMetadata() {
        ...
        Release result = metadataProvider.fetchReleaseMetadata(metadataId, 1).orElseThrow();
